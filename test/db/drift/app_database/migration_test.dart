...
void main() {
  ...
  test('migration from v1 to v2 does not corrupt data', () async {
    ...
    final oldSongData = <v1.SongData>[];
    final expectedNewSongData = <v2.SongData>[];
    ...
    await verifier.testWithDataIntegrity(
      ...
      createItems: (batch, oldDb) {
        ...
        batch.insertAll(oldDb.song, oldSongData);
      },
      validateItems: (newDb) async {
        expect(
            expectedNewCategoryData, await newDb.select(newDb.category).get());
        expect(expectedNewSongData, await newDb.select(newDb.song).get());
      },
    );
  });
}
